...
(defn calc-stat-sum [ tags ]
  (let [
         ...
         defendant-or-plaintiff-tags
           (filter
             ...
             non-nil-tags)
         plaintiff-non-nil-tags
           (keep #(get-in % [:value :plaintiff])
             defendant-or-plaintiff-tags)
        ]
  { :num-judgments (count tags)
    ...
    :num-judgments-with-non-empty-plaintiff
      (count plaintiff-non-nil-tags)}))
  ...
  (println
    (format
       (str "Number of judgments:                             %6d\n"
            ...
            "Number of judgments with non-empty plaintiff:    %6d  (%4.2f %%)")
         (:num-judgments stat)
         ...
         (:num-judgments-with-non-empty-plaintiff stat)
         (float
            (* 100 (/ (:num-judgments-with-non-empty-plaintiff stat)
                      (:num-judgments-with-defendant-or-plaintiff stat)))))))
